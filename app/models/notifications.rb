...
class Notifications < ActionMailer::Base
  ...
  def created(task, user, note = "", sent_at = Time.now)
    ...
    @recipients = []
    @recipients += [user.email] if user.receive_notifications > 0 && user.receive_own_notifications > 0
	...
	if user.receive_own_notifications < 1
  ...
  def changed(update_type, task, user, change, sent_at = Time.now)
    @subject = case update_type
               when :completed  then "#{$CONFIG[:prefix]} #{_'Closed'}: #{task.issue_name} -> #{_(task.status_type)} [#{task.project.name}] (#{user.name})"
