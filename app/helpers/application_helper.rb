...
module ApplicationHelper
  ...
  # I would like to get rid of this dum calendar.png once and for all
...
#      calendar = image_tag(button_image, :id => calendar_id)
  ...
  def link_to_task(task)
	img = nil
	if File.exists?(task.project.customer.logo_path)
		img = "<img style=\"display:none\" src=\"/admin/show_logo/#{task.project.customer_id}\" />"
	end
	"#{img}<strong>#{task.issue_num}</strong> <a href=\"/tasks/edit/#{task.id}\" class=\"tooltip#{task.css_classes}\" title=\"#{task.to_tip({ :duration_format => current_user.duration_format, :workday_duration => current_user.workday_duration, :days_per_week => current_user.days_per_week, :user => current_user })}\">#{h(task.name.chars[0..80])}</a>"
  end
  ...
  def avatar_for(user, size=32)
	if current_user.option_avatars == 1
	  return "<img src=\"#{h(user.avatar_url(size, request.ssl?))}\" class=\"photo\" />"
	end
	""
  end
