...
<table id="schedule" class="#{options[:table_class]}" cellpadding="0" cellspacing="0" border="0" >
  <thead>
   <tr><td width="100%" colspan="7"><div class="month">#{ link_to '<span class="month-nav">&larr;</span>', :controller => 'schedule', :action => 'list', :year => prev_year, :month => prev_month} <span class="month-title">#{_(Date::MONTHNAMES[options[:month]])} #{options[:year]}</span> #{link_to '<span class="month-nav">&rarr;</span>', :controller => 'schedule', :action => 'list', :year => next_year, :month => next_month}</div></td></tr>

   <tr class="#{options[:day_name_class]}">
EOF
    Date::DAYNAMES.each {|d| cal << "                   <th>#{_(d[options[:abbrev]])}</th>"}
    cal << "            </tr>
        </thead>
        <tbody>
                <tr>"
