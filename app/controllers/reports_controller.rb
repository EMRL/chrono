...
class ReportsController < ApplicationController
  ...
  def name_from_worklog(w,r)
    case r
    when 1
      "<span>#{w.task.issue_num}</span> <a href=\"/tasks/view/#{w.task.task_num}\">#{w.task.name}</a> <br /><small>#{w.task.full_name}</small>"
    when 2
  ...
  def do_column(w, key)
    ...
    if @row_value == 2 && !w.task.tags.empty? && (@type == 1 || @type == 4)
      ...
      rkey = key_from_worklog(w, 13).to_s
      row_name = name_from_worklog(w, 15)
      do_row(rkey, row_name, key, "<span>#{w.task.issue_num}</span> <a href=\"/tasks/view/#{w.task.task_num}\">#{w.task.name}</a> <br/><small>#{w.task.full_name}</small>")
    elsif key == "4_user"
      rkey = key_from_worklog(w, 13).to_s
      row_name = name_from_worklog(w, 15)
