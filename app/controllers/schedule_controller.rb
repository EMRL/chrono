...
#      break unless rev
  ...
  def gantt
    ...
    for task in @tasks
	  ...
	  if task.scheduled_start? && (task.scheduled_start != task.start_at)
	    @schedule_in_progress = true
	  end
  ...
  def gantt_reset
    ...
    Task.update_all("scheduled=0, scheduled_at=NULL, scheduled_duration = 0, scheduled_start=NULL", ["tasks.project_id IN (#{projects}) AND tasks.completed_at IS NULL"])
  ...
  def gantt_save
    ...
    tasks.each do |t|
	  ...
	  if t.scheduled_start != t.start_at
	    old_name = "None"
		old_name = current_user.tz.utc_to_local(t.start_at).strftime_localized("%A, %d %B %Y") unless t.start_at.nil?
		
		new_name = "None"
		new_name = current_user.tz.utc_to_local(t.scheduled_start).strftime_localized("%A, %d %B %Y") unless t.scheduled_start.nil?
		
		body << "- <strong>Start</strong>: #{old_name} -> #{new_name}\n"
		t.start_at = t.scheduled_start
	  end
  ...
  def reschedule
	...
	unless @task.scheduled_start?
	  @task.scheduled_start = @task.start_at
	end
    ...
    if params[:due] && params[:due].length > 0
      begin
        ...
        @task.scheduled_at = tz.local_to_utc(due.to_time) unless due.nil?
      rescue
        render :update do |page|
	...
	if params[:start] && params[:start].length > 0
	  begin
	    start = DateTime.strptime( params[:start], current_user.date_format )
		@task.scheduled_start = tz.local_to_utc(start.to_time) unless start.nil?
	  rescue
	    render :update do |page|
		  page["start-#{@task.dom_id}"].value = (@task.scheduled_start ? @task.scheduled_start.strftime_localized(current_user.date_format) : "")
		  page["start-#{@task.dom_id}"].className = ((@task.scheduled_start? && @task.scheduled_start != @task.start_at) ? "scheduled" : "")
		end
		return
	  end
	elsif params[:start]
	  @task.scheduled_start = nil
	end
  ...
  def reschedule_milestone
    ...
    if params[:due] && params[:due].length > 0
      begin
        ...
        @milestone.scheduled_at = tz.local_to_utc(due.to_time) unless due.nil?
      rescue
        render :update do |page|
  ...
  def gantt_drag
    ...
    end_date = start_date + ((params[:w].to_i - 501)/16).days
    ...
    if @milestone
      ...
      @milestone.scheduled_at = tz.local_to_utc(start_date.to_time) unless start_date.nil?
      @milestone.save
    else 
      unless @task.scheduled?
        @task.scheduled_duration = @task.duration
      ...
      @task.scheduled_at = tz.local_to_utc(end_date.to_time) unless end_date.nil?
      @task.save
    end 
